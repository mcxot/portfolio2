...
export const InfiniteMovingCards = ({
  ...
  useEffect(() => {
    if (animationAddedRef.current) return;
    ...
    if (containerRef.current && scrollerRef.current) {
      ...
      // Add animation class directly to DOM
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item) => (
          <li
            className="w-[350px] max-w-full relative rounded-2xl border border-slate-700 px-8 py-6 md:w-[450px] bg-slate-900/50 backdrop-blur-sm"
            key={item.name}
          >
            <blockquote>
              <div
              ...
              ></div>
              {item.rating && (
                <div className="relative z-20 flex gap-1 mb-3">
                  {[...Array(5)].map((_, i) => (
                    <svg
                      key={i}
                      className={`w-4 h-4 ${
                        i < item.rating! ? "text-cyan-400" : "text-slate-600"
                      }`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
